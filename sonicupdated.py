...
sys.path.append("C:\python3664\Lib\site-packages")
import numpy as np
import gym
import retro
import h5py
...
def main():
    ...
    dqn = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=1000,target_model_update=1e-3,
                   policy=policy)


    #chabges made dqn.compile(Adam(lr=1e-3), metrics=['mae'])
